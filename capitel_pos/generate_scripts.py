...
bl = "\\"
dir_name = "DIR_NAME='capitel_pos'_${BATCH_SIZE_PER_GPU}_${WEIGHT_DECAY}_${LEARN_RATE}_$(date +'%m-%d-%y_%H-%M')"
models = [{'m':'beto'},{'m':'ixabertesv1'}, {'m':'ixabertesv2'}, {'m':'ixambert'}, {'m':'mdeberta-v3-base'}, {'m':'xlm-roberta-base'}, {'m':'xlm-roberta-large', 'grad':2} ]
names = {'beto':'dccuchile/bert-base-spanish-wwm-cased','ixambert':'/tartalo03/users/ragerri/resources/ixambert-base-cased', 'ixabertesv2':'/tartalo03/users/ragerri/resources/IXABERTes_v2','ixabertesv1':'/tartalo03/users/ragerri/resources/IXABERTes_v1','mdeberta-v3-base':'microsoft/mdeberta-v3-base','xlm-roberta-base':'xlm-roberta-base','xlm-roberta-large':'xlm-roberta-large'}
    ...
    for batch in batches:
        ...
        for weight in weight_dec:
            for lr in learn_rate:
                with open(f"{model}_capitel_pos_batch{batch}_lr{lr}_decay{weight}.sh", "w") as f:
                    f.write(f"""#!/bin/bash
SEED=42
NUM_EPOCHS=5
...
python ../bsc_run_ner.py --model_name_or_path $MODEL --seed $SEED {bl}
                                         --dataset_script_path ./pos_dataset.py {bl}
                                         --task_name pos --do_train --do_eval --do_predict {bl}
