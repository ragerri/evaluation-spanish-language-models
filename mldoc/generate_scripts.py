...
bl = "\\"
dir_name = "DIR_NAME='mldoc'_${BATCH_SIZE_PER_GPU}_${WEIGHT_DECAY}_${LEARN_RATE}_$(date +'%m-%d-%y_%H-%M')"
models = [{'m':'ixabertesv1'},{'m':'ixabertesv2'},
    ...
    for batch in batches:
        ...
        for weight in weight_dec:
            for lr in learn_rate:
                with open(f"{model}_mldoc_batch{batch}_lr{lr}_decay{weight}.sh", "w") as f:
                    f.write(f"""#!/bin/bash
SEED=42
NUM_EPOCHS=4
...
python ../bsc_run_glue.py --model_name_or_path $MODEL --seed $SEED {bl}
                                         --dataset_script_path ./mldoc_dataset.py {bl}
                                         --task_name mldocs --do_train --do_eval --do_predict {bl}
