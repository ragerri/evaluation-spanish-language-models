...
bl = "\\"
dir_name = "DIR_NAME='sqac'_${BATCH_SIZE_PER_GPU}_${WEIGHT_DECAY}_${LEARN_RATE}_$(date +'%m-%d-%y_%H-%M')"
models = [{'m':'bertin'},{'m':'beto'},{'m':'electricidad'},{'m':'ixabertesv1'},{'m':'ixabertesv2'},
          {'m':'ixambert'},{'m':'bert-base-multilingual-cased'},{'m':'mdeberta-v3-base'}, {'m':'roberta-base-bne'},{'m':'xlm-roberta-base'},
          {'m':'roberta-large-bne', 'grad':2},{'m':'xlm-roberta-large', 'grad':2} ]
names = {'bertin':'bertin-project/bertin-roberta-base-spanish','beto':'dccuchile/bert-base-spanish-wwm-cased','electricidad':'mrm8488/electricidad-base-discriminator','ixambert':'/tartalo03/users/ragerri/resources/ixambert-base-cased','ixabertesv1':'/tartalo03/users/ragerri/resources/IXABERTes_v1','ixabertesv2':'/tartalo03/users/ragerri/resources/IXABERTes_v2','bert-base-multilingual-cased':'bert-base-multilingual-cased','mdeberta-v3-base':'microsoft/mdeberta-v3-base','roberta-base-bne':'PlanTL-GOB-ES/roberta-base-bne','roberta-large-bne':'PlanTL-GOB-ES/roberta-large-bne','xlm-roberta-base':'xlm-roberta-base','xlm-roberta-large':'xlm-roberta-large'}
    ...
    for batch in batches:
        ...
        for weight in weight_dec:
            for lr in learn_rate:
                with open(f"{model}_sqac_batch{batch}_lr{lr}_decay{weight}.sh", "w") as f:
                    f.write(f"""#!/bin/bash
SEED=42
NUM_EPOCHS=4
...
python ../run_qa.py --model_name_or_path $MODEL --seed $SEED {bl}
                                         --dataset_name PlanTL-GOB-ES/SQAC {bl}
                                         --do_train --do_eval --do_predict {bl}
